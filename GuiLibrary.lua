--rektsky library ATM, custom later
...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		spawn(function()
			...
			textlabel.Text = "Setting up Moon, downloaded "..path
			textlabel.BackgroundTransparency = 1
			textlabel.TextStrokeTransparency = 0
			textlabel.TextSize = 30
			textlabel.Font = Enum.Font.SourceSans
			textlabel.TextColor3 = Color3.new(0.576471, 0.0627451, 1)
...
local function createnotification(title, text, delay2, toggled)
	spawn(function()
		...
		frame.Position = UDim2.new(1, -115, 1, -150) -- Adjusted position
		frame.BorderSizePixel = 0
		frame.AnchorPoint = Vector2.new(1, 1) -- Adjusted anchor point
		...
		pcall(function()
			frame:TweenPosition(UDim2.new(1, -115, 1, -150), Enum.EasingDirection.InOut, Enum.EasingStyle.Quad, 0.15) -- Adjusted tween position
			game:GetService("Debris"):AddItem(frame, delay2 + 0.15)
		end)
	end)
end

createnotification("Moon Loaded", "Press Insert to toggle GUI", 3, true)
